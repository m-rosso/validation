...
class KfoldsCV(object):
    ...
    _AVAILABLE_TASKS = ['regression', 'classification', 'binary', 'binary:logistic']
    _AVAILABLE_METHODS = ['logistic_regression', 'GBM', 'random_forest', 'SVM', 'lasso', 'light_gbm', 'xgboost']
    _METHODS_DICT_CLASS = {'logistic_regression': 'LogisticRegression', 'GBM': 'GradientBoostingClassifier',
                          'random_forest': 'RandomForestClassifier', 'SVM': 'SVC'}
    _METHODS_DICT_REG = {'lasso': 'Lasso', 'GBM': 'GradientBoostingRegressor',
                        'random_forest': 'RandomForestRegressor', 'SVM': 'SVR'}
    
    def __init__(self, task: str = 'classification', method: str = 'logistic_regression',
                 metric: str = 'roc_auc', num_folds: int = 3, shuffle: bool = False, stratified: bool = False,
                 pre_selecting: bool = False, pre_selecting_params: Optional[dict] = None,
                 random_search: bool = False, n_samples: Optional[int] = None,
                 grid_param: Optional[dict] = None, default_param: Optional[dict] = None, fixed_params: Optional[dict] = None,
        ...
        f""
        if self.task not in self._AVAILABLE_TASKS:
            raise ValueError(f'Please, choose task among {self._AVAILABLE_TASKS}.')
        if self.method not in self._AVAILABLE_METHODS:
            raise ValueError(f'Please, choose method among {self._AVAILABLE_METHODS}.')
        if self.metric not in list(self.__METRICS_FUNCTIONS.keys()):
            raise ValueError(f'Please, choose metric among {list(self.__METRICS_FUNCTIONS.keys())}.')
    ...
    def run(self, inputs: pd.DataFrame, output: pd.DataFrame, progress_bar: bool = True, print_outcomes: bool = True,
        ...
        self.CV_metric = pd.DataFrame() # Object for storing performance metrics for each combination of hyper-parameters.
        self._CV_metric_folds = {} # Object for storing performance metrics for each combination of hyper-parameters at each fold.
        CV_scores = dict(zip([str(g) for g in self.grid_param],
                              [pd.DataFrame(data=[], columns=['cv_score']) for i in
                              range(len(self.grid_param))])) # Object for storing CV estimated scores.
        ...
        for j in range(len(self.grid_param)):
            CV_metric_list = [] # Object for storing performance metrics for each combination of hyper-parameters.
            ...
            try:
                ...
                print(f'\033[1mNot able to perform CV estimation with parameters {self.grid_param[j]}!\033[0m')
                self.CV_metric = pd.concat([self.CV_metric,
                                            pd.DataFrame(data={'tun_param': str(self.grid_param[j]),
                                                                'cv_' + self.metric: np.NaN},
                                                          index=[j])], axis=0, sort=False) # Dataframes with CV performance metrics.
                print(f'Error: {e}')
        ...
        try:
            ...
            self._CV_metric_folds = self._CV_metric_folds[self.best_param] # Metric of best hyper-parameters for each fold.
            self.best_param = self.grid_param[self.best_param] # Best hyper-parameters.
            self.__using_default = False
            self.CV_scores = CV_scores[str(self.best_param)] # CV scores for best tuning parameter.
            
        except:
            self.best_param = self.default_param # Best (default) hyper-parameters.
    ...
    def __splitting_data(self, inputs, output, num_folds, shuffle, stratified):
        if (shuffle==True) & (stratified==False):
    ...
    def __run_estimation(self, folds_X, folds_y, fold_idx, param_idx, CV_scores_dict):
        ...
        if self.method=='light_gbm':
            score_pred = self.__create_run_lgb(train_inputs=X_train, train_output=y_train, test_inputs=X_val,
                                              test_output=y_val, param_idx=param_idx)

        elif self.method=='xgboost':
            score_pred = self.__create_run_xgb(train_inputs=X_train, train_output=y_train, test_inputs=X_val,
                                              test_output=y_val, param_idx=param_idx)
    ...
    # Function that creates, trains and returns predictions for LightGBM models:
    def __create_run_lgb(self, train_inputs, train_output, test_inputs, test_output, param_idx):
    ...
    # Function that creates, trains and returns predictions for XGBoost models:
    def __create_run_xgb(self, train_inputs, train_output, test_inputs, test_output, param_idx):
        # Create dictionary with parameters:
        param = {'objective': self.task,
    ...
    def __create_model(self, task, method, params):
        ...
        if task=='classification':
            return eval(f'{self._METHODS_DICT_CLASS[method]}({init_params})')

        if task=='regression':
            return eval(f'{self.METHODS_DICT_REG[method]}({init_params})')
...
class Kfolds_fit(KfoldsCV):
    ...
    def __init__(self, task: str = 'classification', method: str = 'logistic_regression',
                 metric: str = 'roc_auc', num_folds: int = 3, shuffle: bool = False, stratified: bool = False,
                 pre_selecting: bool = False, pre_selecting_params: Optional[dict] = None, only_final_selection: bool = False,
                 random_search: bool = False, n_samples: Optional[int] = None,
                 grid_param: Optional[dict] = None, default_param: Optional[dict] = None, fixed_params: Optional[dict] = None,
                 parallelize: bool = False) -> None:                 
        KfoldsCV.__init__(self, task, method, metric, num_folds, shuffle, stratified, pre_selecting, pre_selecting_params,
    ...
    def fit(self, train_inputs: pd.DataFrame, train_output: pd.DataFrame,
            val_inputs: Optional[pd.DataFrame] = None, val_output: Optional[pd.DataFrame] = None,
            test_inputs: Optional[pd.DataFrame] = None, test_output: Optional[pd.DataFrame] = None,
            progress_bar: bool = True, print_outcomes: bool = True, print_time: bool = True) -> None:
        """
        Method that runs K-folds CV estimation, refits using all training data, and evaluate performance metrics on
        test data (when provided).
        ...
        if (self.pre_selecting) | (self.only_final_selection):
            self.selected_features = self._KfoldsCV__pre_selection(input_train=train_inputs, output_train=train_output,
                                                                   pre_selecting_params=self.pre_selecting_params)
            self.num_selected_features = len(self.selected_features)
            
            train_inputs = train_inputs[self.selected_features]
            
            if test_inputs is not None:
                test_inputs = test_inputs[self.selected_features]

        # Train-test estimation:
        if self.method=='light_gbm':
            self.__run_lgb(train_inputs=train_inputs, train_output=train_output,
                          val_inputs=val_inputs, val_output=val_output,
                          test_inputs=test_inputs)

        elif self.method=='xgboost':
            self.__run_xgb(train_inputs=train_inputs, train_output=train_output,
                          val_inputs=val_inputs, val_output=val_output,
                          test_inputs=test_inputs)
    ...
    # Method for creating and training a LightGBM model:
    def __run_lgb(self, train_inputs, train_output, val_inputs, val_output, test_inputs):
        if self.method == 'light_gbm':
            # Create dictionary with parameters:
            param = {'objective': self.task,
    ...
    # Method for creating and training a XGBoost model:
    def __run_xgb(self, train_inputs, train_output, val_inputs, val_output, test_inputs):
        # Create dictionary with parameters:
        param = {'objective': self.task,
